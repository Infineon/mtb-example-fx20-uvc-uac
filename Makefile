...
LPBK_EN?=no
ifeq ($(LPBK_EN), no)
    DEFINES += LVDS_LB_EN=0 FPGA_ENABLE=1 LVCMOS_EN=1 LVCMOS_DDR_EN=1 WL_EN=1 INTERLEAVE_EN=0 PORT1_EN=0 FPGA_ADDS_HEADER=0 AUDIO_IF_EN=1 STEREO_ENABLE=0
else
    DEFINES += LVDS_LB_EN=1 FPGA_ENABLE=0 LVCMOS_EN=0 LVCMOS_DDR_EN=0 WL_EN=0 INTERLEAVE_EN=0 PORT1_EN=0 FPGA_ADDS_HEADER=0 AUDIO_IF_EN=1 STEREO_ENABLE=0 
endif

# Compile for Gen2 operation unless GEN1=yes switch is used.
GEN1?=no
ifeq ($(GEN1), yes)
    DEFINES += USBSS_GEN2_ENABLE=0
else
    DEFINES += USBSS_GEN2_ENABLE=1
endif

# Compile for dual USB lane operation unless SINGLELANE=yes switch is used.
SINGLELANE?=no
ifeq ($(SINGLELANE), yes)
    DEFINES += USBX2_EN=0
else
...
ifeq ($(TOOLCHAIN), GCC_ARM)
    ...
    LDFLAGS=--cpu=Cortex-M4 --entry=Reset_Handler --diag_suppress=L6314W,L6329W
endif

# Additional / custom libraries to link in to the application.
LDLIBS=

# Use loadable by default
BLENABLE?=yes
ifeq ($(BLENABLE), yes)
    DEFINES += BLOAD_ENABLE=1
else
    DEFINES += BLOAD_ENABLE=0
endif
